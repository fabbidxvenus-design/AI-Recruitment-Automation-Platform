    ...
    try {
      if (importSource === 'manual' && selectedFiles.length > 0) {
        for (const file of selectedFiles) {
          const result = await cvService.importCV({
          ...
          if (!result.success || !result.data) {
            console.error('[CVImport] Import failed for file:', file.name, 'Error:', result.error);
            const importErrorMessage = typeof result.error === 'string' ? result.error : result.error?.message;
            setImportError(importErrorMessage || t('cvImport.error.body', { defaultValue: 'File import failed. Please try again.' }));
            return;
          }
        }
      ...
      console.error('[CVImport] Import failed:', err);
      setImportError(t('cvImport.error.body', { defaultValue: 'File import failed. Please try again.' }));
    } finally {
      setImporting(false);
    }
  };
            ...
            <div className={styles.timeline}>
              <div className={styles.timelineItem}>
                <div className={styles.timelineMarker}>
                  <StatusBadge variant="success" label={t('common.completed')} />
                </div>
              ...
              {rescreenRequested && (
                <div className={styles.timelineItem}>
                  <div className={styles.timelineMarker}>
                    <StatusBadge variant="info" label={t('cvImport.timeline.pending')} />
                  </div>
                  <div className={styles.timelineContent}>
              ...
              <div className={styles.timelineItem}>
                <div className={styles.timelineMarker}>
                  <StatusBadge variant={rescreenRequested ? 'warning' : 'info'} label={t('cvImport.timeline.inProgress')} />
                </div>
                <div className={styles.timelineContent}>
        ...
        <div id="cv-import-panel-manual" role="tabpanel" aria-labelledby="cv-import-tab-manual">
          ...
          <div className={styles.actions}>
            {versionCreated && !rescreenRequested && (
              <Button variant="secondary" aria-label={t('common.rescreenAria')} onClick={() => setRescreenRequested(true)}>
                {t('cvImport.rescreen.action')}
              </Button>
            )}
            <Button
              variant="primary"
              aria-label={t('common.scanAria')}
      ...
      {importSource === 'drive' && (
        <div id="cv-import-panel-drive" role="tabpanel" aria-labelledby="cv-import-tab-drive">
          ...
          <div className={styles.actions}>
            ...
            <Button variant="primary" onClick={handleImport} disabled={!driveFolder || importing} aria-label={t('cvImport.drive.scanAria')}>
