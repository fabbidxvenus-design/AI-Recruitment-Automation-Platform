            ...
            <CardContent>
              ...
              {isLoading ? (
                ...
                <div
                  ...
                  role="status"
                  aria-live="polite"
                  aria-label={t('screening.review.empty.noMatch')}
                >
                  <span className={styles.emptyIcon} aria-hidden="true" role="img" aria-label="Clipboard with checkmarks">&#x1F4CB;</span>
                  <p>{t('screening.review.empty.noMatch')}</p>
                </div>
              ) : (
                <ul
                  className={styles.candidateItems}
                  aria-label={t('screening.review.candidates')}
                >
                  {filteredEvaluations.map(evaluation => (
                    ...
                    >
                      <label className={styles.checkbox} htmlFor={`select-candidate-${evaluation.id}`} onClick={(e) => e.stopPropagation()}>
                        <input
                          type="checkbox"
                          id={`select-candidate-${evaluation.id}`}
          ...
          {isLoading ? (
            ...
            <Card>
                  ...
                  <section className={styles.evaluationSection}>
                    ...
                    <ul className={styles.strengthsList}>
                      {selected.keyStrengths.map((strength, i) => (
                        <li key={`strength-${strength}-${i}`} className={styles.strengthItem}>
                          <span className={styles.strengthIcon} aria-hidden="true" role="img">&#x2713;</span>
                          {strength}
                        </li>
                      ))}
                    </ul>
                  ...
                  <section className={styles.evaluationSection}>
                    ...
                    <ul className={styles.concernsList}>
                      {selected.concerns.map((concern, i) => (
                        <li key={`concern-${concern}-${i}`} className={styles.concernItem}>
                          <span className={styles.concernIcon} aria-hidden="true" role="img">&#x26A0;</span>
                          {concern}
                        </li>
                      ))}
                    </ul>
              ...
              <div className={styles.actionButtons}>
                <Button variant="danger" aria-label={t('common.reject')} onClick={handleRejectSelected}>
                  {t('screening.review.actions.reject')}
                </Button>
                <Button variant="secondary" aria-label={t('cvImport.rescreen.action')} onClick={handleReevaluation}>
                  {t('screening.review.actions.rescreening')}
                </Button>
                <Button variant="primary" aria-label={t('common.approve')} onClick={handleStageAdvance}>
            ...
            <Card>
              <CardContent>
                <div className={styles.emptyState}>
                  <span className={styles.emptyIcon} aria-hidden="true" role="img" aria-label="Pointing hand">&#x1F446;</span>
