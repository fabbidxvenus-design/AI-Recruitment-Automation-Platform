...
export default function AIDesignPage() {
  ...
  const handleGenerate = async (): Promise<void> => {
    ...
    try {
      ...
      if (!isValidBrief(brief)) {
        setValidationErrors([t('common.formValidation.requiredFields')]);
        setStatus('empty');
        return;
      }
      ...
      console.error('[aiDesign] Design generation failed', error);
      setStatus('empty');
      const errorMessage = error instanceof Error ? error.message : t('tools.aiDesign.workspace.generationFailed');
  ...
  const handleRequestApproval = async (): Promise<void> => {
    ...
    try {
      ...
      console.error('[aiDesign] Approval request failed', error);
      setStatus('generated');
      const errorMessage = error instanceof Error ? error.message : t('tools.aiDesign.workspace.approvalFailed');
      setValidationErrors([errorMessage]);
    }
  };
  ...
  const handleApprove = (): void => {
    ...
    const updatedHistory = approvalHistory.map((record) =>
      record.status === 'pending'
        ? { ...record, status: 'approved' as const, approver: t('common.role.hiringManager'), timestamp: new Date().toISOString() }
        : record
    );
    ...
    const updatedHistory = approvalHistory.map((record) =>
      record.status === 'pending'
        ? {
            ...
            approver: t('common.role.hiringManager'),
            reason,
            timestamp: new Date().toISOString(),
          }
        : record
  ...
  const handleExport = async (): Promise<void> => {
    ...
    try {
      ...
      console.error('[aiDesign] Export failed', error);
            ...
            <form className={styles.form} onSubmit={(e) => e.preventDefault()}>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  required
                  aria-required="true"
                  aria-invalid={touchedFields.jobTitle && (brief.jobTitle?.trim().length ?? 0) === 0}
                />
              </div>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  required
                  aria-required="true"
                  aria-invalid={touchedFields.companyName && (brief.companyName?.trim().length ?? 0) === 0}
                />
              </div>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  onBlur={() => setTouchedFields((prev) => ({ ...prev, keyMessage: true }))}
                  required
                  aria-required="true"
                  aria-invalid={touchedFields.keyMessage && (brief.keyMessage?.trim().length ?? 0) === 0}
                />
              </div>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  onBlur={() => setTouchedFields((prev) => ({ ...prev, targetAudience: true }))}
                  required
                  aria-required="true"
                  aria-invalid={touchedFields.targetAudience && (brief.targetAudience?.trim().length ?? 0) === 0}
                />
              </div>
        ...
        <div className={styles.workspace}>
          ...
          <div className={styles.workspaceGrid}>
            <Card className={styles.variantsPanel}>
              <CardHeader title={t('tools.aiDesign.workspace.variants')} description={t('tools.aiDesign.workspace.variantCount', { count: variants.length })} />
              <CardContent>
                      ...
                      <CardContent>
                        ...
                        {status === 'pending_approval' && (
                          <div className={styles.approvalActions}>
                            <Button onClick={handleApprove} variant="primary" aria-label={t('tools.aiDesign.approval.approveAria', { defaultValue: 'Approve design' })}>
                              {t('tools.aiDesign.approval.approve')}
                            </Button>
                            <Button onClick={() => handleReject(t('tools.aiDesign.approval.defaultRejectionReason'))} variant="danger" aria-label={t('tools.aiDesign.approval.rejectAria', { defaultValue: 'Reject design' })}>
