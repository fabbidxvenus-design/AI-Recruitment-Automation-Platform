...
export default function ErrorDetailPage() {
  ...
  return (
    <main id="main-content" className={styles.errorDetailPage}>
      ...
      <div className={`${styles.severityBanner} ${severityClass}`}>
        ...
        <div className={styles.metaInfo}>
          <span className={styles.metaLabel}>{t('errors.detail.meta.created')}</span>
      ...
      <div className={styles.mainContent}>
          ...
          <Card>
            ...
            <CardContent>
              <div className={styles.entityList}>
                {error.affectedEntities.map((entityId) => (
                  <div key={`entity-${entityId}`} className={styles.entityItem}>
                ...
                {retryAttempts.length > 0 ? (
                  <div className={styles.retryHistoryTableContainer}>
                    <table className={styles.retryHistoryTable}>
                      <thead>
                        <tr>
                          <th scope="col">{t('errors.detail.retryConfig.retryHistory.attempt')}</th>
                          <th scope="col">{t('errors.detail.retryConfig.retryHistory.timestamp')}</th>
                          <th scope="col">{t('errors.detail.retryConfig.retryHistory.status')}</th>
                          <th scope="col">{t('errors.detail.retryConfig.retryHistory.errorMessage')}</th>
                          <th scope="col">{t('errors.detail.retryConfig.retryHistory.duration')}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {retryAttempts.map((attempt) => (
                          <tr key={attempt.id} className={styles.retryRow}>
                            ...
                            <td>{attempt.status === 'failed' ? t('errors.detail.retryConfig.retryHistory.failedDuration') : '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        <div className={styles.contentRight}>
                ...
                <div className={styles.settingsCard}>
                  ...
                  <div className={styles.settingsGrid}>
                    <div className={styles.settingItem}>
                      ...
                      <span className={`${styles.settingValue} ${styles.danger}`}>{t('errors.detail.retryConfig.retryHistory.maxAttemptsReached')}</span>
                    </div>
                    <div className={styles.settingItem}>
                ...
                <div className={styles.previousStateCard}>
                  ...
                  <div className={styles.previousStateGrid}>
                    <div className={styles.settingItem}>
                      ...
                      <span className={styles.settingValue}>{t('errors.detail.retryConfig.enhancedPreviousState.interviewScheduled')}</span>
                    </div>
                    <div className={styles.settingItem}>
                      <span className={styles.settingLabel}>{t('errors.detail.retryConfig.enhancedPreviousState.lastSuccess')}</span>
                      <span className={styles.settingValue}>
                        {t('errors.detail.retryConfig.enhancedPreviousState.lastVideo')}<br/>
                        <small>{t('errors.detail.retryConfig.enhancedPreviousState.lastVideoTime')}</small>
                      </span>
                    </div>
                    <div className={styles.settingItem}>
                      <span className={styles.settingLabel}>{t('errors.detail.retryConfig.enhancedPreviousState.affectedCount')}</span>
                      <span className={`${styles.settingValue} ${styles.danger}`}>{t('errors.detail.retryConfig.enhancedPreviousState.candidatesCount')}</span>
                    </div>
                  </div>

                  <div className={styles.stateComparison}>
                    <div className={styles.stateBox}>
                      <span className={`${styles.stateTag} ${styles.before}`}>{t('errors.detail.beforeAfter.before')}</span>
                      <span>{t('errors.detail.beforeAfter.processingQueue')}</span>
                    </div>
                    <div className={styles.stateArrow}>→</div>
                    <div className={styles.stateBox}>
                      <span className={`${styles.stateTag} ${styles.after}`}>{t('errors.detail.beforeAfter.after')}</span>
                      <span>{t('errors.detail.beforeAfter.errorState')}</span>
                    </div>
                  </div>
                </div>
          ...
          <div className={styles.escalationWarning}>
            <div className={styles.escalationIcon} aria-hidden="true">⚠️</div>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className={styles.actionButtons}>
                <Button variant="secondary" aria-label={t('errors.detail.actions.manualRetry')} onClick={() => setShowRetryModal(true)}>
                  <span aria-hidden="true">🔄</span> {t('errors.detail.actions.manualRetry')}
                </Button>
                <Button variant="danger" aria-label={t('errors.detail.actions.escalateNow')} onClick={() => setShowEscalateModal(true)}>
