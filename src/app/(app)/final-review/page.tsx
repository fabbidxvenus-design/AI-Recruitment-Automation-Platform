...
export default function FinalReviewPage() {
  ...
  const selected = finalReviewPackages.find(p => p.id === selectedCandidate) || activeWorkflowPackage || finalReviewPackages[0];
  ...
  const handleSubmitDecision = () => {
    ...
    if (mfaVerified && reason.trim()) {
      ...
      setFeedback({
        variant: 'success',
        ...t('finalReview.feedback.decisionSubmitted', { name: selected.candidateName, returnObjects: true }) as { title: string; body: string },
      });
  ...
  return (
    <main id="main-content" className={styles.finalReviewPage}>
          ...
          <Card>
                  ...
                  <div className={styles.documentList}>
                    {selected.documents.map(doc => (
                      <div key={doc.id} className={styles.documentItem}>
                        ...
                        <Button variant="ghost" size="sm" aria-label={t('finalReview.documentActions.viewAria', { name: doc.name })} onClick={() => setViewedDocument(doc.name)}>{t('finalReview.documentActions.view')}</Button>
                      </div>
                    ))}
                  </div>
                </section>
            ...
            <div className={styles.actionButtons}>
              <Button
                ...
                onClick={() => setFeedback({
                  variant: 'info',
                  title: t('finalReview.feedback.infoRequested.title'),
                  body: t('finalReview.feedback.infoRequested.body', { name: selected.candidateName }),
                })}
              >
        ...
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              <h2>{t('finalReview.history.title')}</h2>
              <button className={styles.closeButton} onClick={() => setHistoryOpen(false)} aria-label={t('common.closeModal')}>×</button>
            </div>
            ...
            <div className={styles.modalFooter}>
              <Button variant="primary" onClick={() => setHistoryOpen(false)}>{t('common.close')}</Button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              ...
              <button className={styles.closeButton} onClick={() => setViewedDocument(null)} aria-label={t('common.closeModal')}>×</button>
            </div>
            <div className={styles.modalBody}>
            ...
            <div className={styles.modalFooter}>
              <Button variant="primary" onClick={() => setViewedDocument(null)}>{t('common.close')}</Button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              ...
              <button className={styles.closeButton} onClick={() => setShowDecisionModal(false)} aria-label={t('common.closeModal')}>×</button>
            </div>
            <div className={styles.modalBody}>
