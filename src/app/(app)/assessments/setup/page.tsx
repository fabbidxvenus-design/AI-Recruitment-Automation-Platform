...
function formatRelativeTime(isoString: string, t: (key: string, options?: Record<string, unknown>) => string): string {
  ...
  if (diffDays === 0) return t('common.dateTime.today');
  if (diffDays === 1) return t('common.dateTime.yesterday');
  if (diffDays < 7) return t('common.dateTime.daysAgo', { count: diffDays });
  if (diffDays < 30) return t('common.dateTime.weeksAgo', { count: Math.floor(diffDays / 7) });
  return t('common.dateTime.monthsAgo', { count: Math.floor(diffDays / 30) });
}
...
export default function AssessmentSetupPage() {
  ...
  const workflowEvents = useMemo(() => buildWorkflowEvents(localStatus, selectedPlanVersion, t), [localStatus, selectedPlanVersion, t]);
    ...
    <div id="main-content" className={styles.container}>
      ...
      <div className={styles.workflowGrid}>
        ...
        <Card className={styles.workflowCard}>
          ...
          <h2 className={styles.sectionTitle}>{localStatus === 'approved' ? t('common.role.hiringManager') : t('common.pending')}</h2>
            ...
            <div key={idx} className={styles.timelineItem}>
              ...
              <div className={styles.timelineContent}>
                <div className={styles.timelineHeader}>
                  ...
                  <span className={styles.timelineTime}>{formatRelativeTime(event.timestamp, t)}</span>
                </div>
