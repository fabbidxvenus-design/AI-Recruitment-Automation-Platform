...
export default function TestGradingPage() {
  ...
  const handleOverrideSubmit = () => {
    ...
    setFeedback({
      variant: 'success',
      ...t('tests.grading.feedback.override', { name: selected.candidateName, score: nextScore, maxScore: selected.maxScore, returnObjects: true }) as { title: string; body: string },
    });
  ...
  const handleFlagForReview = () => {
    ...
    setFeedback({
      variant: 'warning',
      ...t('tests.grading.feedback.flagged', { name: selected.candidateName, returnObjects: true }) as { title: string; body: string },
    });
  };

  return (
    <main id="main-content" className={styles.gradingPage}>
      ...
      <div className={styles.tabs} role="tablist" aria-label={t('tests.grading.tabs.filterLabel')}>
        {(['all', 'mcq', 'essay', 'coding'] as TabType[]).map(tab => (
          <button
            ...
            aria-controls="grading-test-results"
            tabIndex={activeTab === tab ? 0 : -1}
            onClick={() => setActiveTab(tab)}
            onKeyDown={(e) => {
              const tabs: TabType[] = ['all', 'mcq', 'essay', 'coding'];
              ...
              document.getElementById(`grading-tab-${tabs[nextIndex]}`)?.focus();
            }}
          >
      ...
      <div id="grading-test-results" role="tabpanel" aria-labelledby={`grading-tab-${activeTab}`}>
              ...
              <div className={styles.testItems}>
                {filteredTests.map(test => (
                  <button
                    ...
                    aria-label={`${test.testName}, ${test.candidateName}, ${test.score} ${t('tests.grading.aria.outOf', { max: test.maxScore })}`}
                  >
                    <div className={styles.testHeader}>
                      ...
                      <StatusBadge
                        ...
                        label={t(`common.status.${test.status}`, { defaultValue: test.status })}
                      />
                    </div>
                    ...
                    <div className={styles.infoGrid}>
                      ...
                      <div className={styles.infoItem}>
                        ...
                        <StatusBadge
                          ...
                          label={t(`common.status.${selected.status}`, { defaultValue: selected.status })}
                        />
                      </div>
                      <div className={styles.infoItem}>
                        <span className={styles.infoLabel}>{t('tests.grading.gradingInfo.passThreshold')}</span>
                        <span className={styles.infoValue}>{t('tests.grading.gradingInfo.passThresholdValue')}</span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className={styles.scoreBreakdown}>
                      ...
                      <div className={styles.sectionScores}>
                        <div className={styles.sectionScore}>
                          ...
                          <ProgressBar value={Math.min(selected.score + 5, 100)} variant="info" size="sm" label={t('tests.grading.scoreBreakdown.accuracy')} />
                        </div>
                        <div className={styles.sectionScore}>
                          <span className={styles.sectionLabel}>{t('tests.grading.scoreBreakdown.completeness')}</span>
                          <ProgressBar value={Math.max(selected.score - 10, 0)} variant="info" size="sm" label={t('tests.grading.scoreBreakdown.completeness')} />
                        </div>
                        <div className={styles.sectionScore}>
                          <span className={styles.sectionLabel}>{t('tests.grading.scoreBreakdown.quality')}</span>
                          <ProgressBar value={selected.score - 5} variant="info" size="sm" label={t('tests.grading.scoreBreakdown.quality')} />
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      {overrideModalOpen && (
        <div className={styles.modal} aria-modal="true" role="dialog" aria-labelledby="override-modal-title" aria-describedby="override-modal-desc">
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              <h2 id="override-modal-title">{t('tests.grading.modal.title')}</h2>
              <button className={styles.closeButton} onClick={() => setOverrideModalOpen(false)} aria-label={t('common.closeModal')}><span aria-hidden="true">×</span></button>
            </div>
            <div className={styles.modalBody}>
              <p id="override-modal-desc" className={styles.modalDescription}>
            ...
            <div className={styles.modalFooter}>
              ...
              <Button variant="primary" onClick={handleOverrideSubmit} disabled={!overrideReason.trim()}>
                {t('tests.grading.modal.submitOverride')}
              </Button>
            </div>
          </div>
        </div>
